...
var $reviewMsg = $('#error_msg_review');
$.validate({ 	
	language : jsLanguageValidator,
        form : '#forms', 
        errorMessagePosition: $reviewMsg,
...
var $errPlace = $('#has_error_message_signup');
$.validate({ 	
	language : jsLanguageValidator,
    form : '#form-signup',
    errorMessagePosition: $errPlace,
    errorMessageClass: 'invalid-error',
...
var $partner = $('#partner-error');
$.validate({ 	
	language : jsLanguageValidator,
    form : '#form-partner',
    errorMessagePosition: $partner,
    errorMessageClass: 'invalid-error',
    scrollToTopOnError : false,
    onError : function() {      
    },
    onSuccess : function() {           
      form_submit('form-partner');
      return false;
    }  
});
...
function busy(e)
{
    ...
    if (e) {
      ...
      $("body").before("<div style=\"padding-left: 46%;position:fixed ;padding-top: 23%;z-index: 12000; background:#666; opacity:0.5; width:100%; height:100%;\" class=\"preloader\"><img src='"+sites_url+"/assets/images/bhukkasloader.gif'  style=\"width:10% !important\"/></div>"); 
...
function addQtyItem(item_id,row,mid,qty,price,i){
    
    //console.log('addQtyItem',item_id,row,mid,qty);
    qty = parseInt(qty);
    
    //console.log(row);
    //console.log(qty);
    var size = $("#cart_"+i).val();
    if(qty < 1){
        delete_item(row);
        qty=0;
    }
    ...
    var totalQty =  qty+1;
    var rw = row+1;
    var params="action=addToCart&currentController=store&item_id="+item_id+"&row="+rw+"&merchant_id="+mid+"&qty="+totalQty+"&price="+price+"|"+size;
      $.ajax({    
        type: "POST",
        url: ajax_url,
        data: params,
...
function removeQtyItem(item_id,row,mid,qty,price,i){
    
    //console.log('removeQtyItem',item_id,row,mid,qty);
    var size = $("#cart_"+i).val();
    var totalQty =  qty-1;
        ...
        success: function(data){ 
        	...
        	if (data.code==1){
        		...
        		if ( action=="forgotPassword"){
                            $(".has_error_msg").html("<p class='alert alert-success'>"+data.msg+"</p>");
        		...
        		if ( $("#redirect").length>=1 ){
        			if (typeof data.details === "undefined" || data.details==null ) {   
        			    ...
        			    uk_msg_sucess(data.msg);
                                    setTimeout(function(){ window.location.replace(  $("#redirect").val() ); }, 3000);
                                    return;
                                    
        			} else {
                                    uk_msg_sucess(data.msg);
                                    setTimeout(function(){ window.location.replace(  $("#redirect").val()+"/id/"+data.details ); }, 3000);
        		            return;
        			}
        		}
        		...
        		if ( action == "placeOrder" ){
        			...
        			}else if(data.details.payment_type =="cca")	{
        				window.location.replace(sites_url+"/store/ccainit/id/"+data.details.order_id);							
        			} else{
        		...
        		if (action=="merchantSignUp"){
        			...
        			/*if ( $("#merchant_email_verification").val()=="yes" ){
                        ...
                        if (action=="merchantRegister"){
        			window.location.replace(sites_url+"/store/MerchantRegister/Do/step2/token/"+data.details);
        			/*if ( $("#merchant_email_verification").val()=="yes" ){
        				window.location.replace(sites_url+"/store/merchantSignup/Do/thankyou/token/"+data.details);
        			} else {
        				window.location.replace(sites_url+"/store/merchantSignup/Do/step3/token/"+data.details);
                        ...
                        if(action=="merchantAccountInfo"){
                           window.location.replace(sites_url+"/store/MerchantRegister/Do/thankyou1/token/"+data.details); 
                        }
...
//        			//console.debug("setAddress");
//                                uk_msg('Record successfully added');
//        			//window.location.href=sites_url+"/store/addressbook";
//        			return;
//        		}
        		...
        		uk_msg(data.msg);
                        $(".has_error_msg").html("<p class='alert alert-danger'>"+data.msg+"</p>");
                        setTimeout(function(){$(".has_error_msg").html("");}, 3000);
...
jQuery(document).ready(function() {
  ...
  if( $('#table_list').is(':visible') ) {    	
    ...
    if( $('#bar-rating').is(':visible') ) {	
      ...
      $( document ).on( "change", "#bar-ratings", function() {
    	var ratings_new = $(this).val();
        $("#initial_review_rating").val(ratings_new);
      });
   ...
   $( document ).on( "click", ".delete_item", function() {  
           ...
           bootbox.confirm(js_lang.trans_4, function(results) {
           if(results){
               var row=$(".delete_item").data("row");   	   
   	       delete_item(row);
           }
           });
   ...
   $( document ).on( "click", ".checkout", function() {    	  
   	 ...
   	 if ( $("#delivery_type").val()=="pickup"){   	     
   	  ...
   	  var params="delivery_type="+$("#delivery_type").val()+"&delivery_date="+$("#delivery_date1").val();
   ...
   $( document ).on( "click", ".remove-rating", function() {  
       /*var r = confirm("Are you sure want to remove rating?");*/
        if (true) {
            
             var params="action=editRatingsReview&currentController=store&merchant_id="+$("#merchant_id").val();
	    ...
	    success: function(data){ 
	    	busy(false);      
	    	if (data.code==1){
                    $("#editMyReview").modal('show');
                    //$("#reviewId").val(data.details['id']);
                    //$("#review_content_text").val(data.details['review']);
                    uk_msg_sucess(data.msg);
	    	} else {
	    		uk_msg_sucess(data.msg);
	    	}	    
  ...
  $(".review-content-wrap").hide("fast");
   $( document ).on( "click", ".write-review", function() {   
    ...
    $("#bar-ratings").hide("fast");
     $( document ).on( "click", ".write-ratings", function() {   
        
   	   if ( $(this).hasClass("active") ){
   	   	   ////console.debug('d2');
   	   	   $("#bar-ratings").slideToggle("fast");
   	   } else {   
   	       $("#bar-ratings").slideToggle("fast");
   	   }
   });
   ...
   /* if ( $(".menu-wrapper").is(":visible") ) {      	   	    
    	...
    	initializeMarker(locations);    sdfsdf  
   } */

   $( document ).on( "click", ".maping", function() {    	     	  
   	  var locations;
   ...
   $( document ).on( "click", "#filters_col_cu", function() {    	       	 	

   	  $("#cusine_list").slideToggle("fast");
   });
   
    $( document ).on( "click", "#filters_col_min_order", function() {    	       	 	

   	  $("#min_order_list").slideToggle("fast");
   });
   $( document ).on( "click", "#filters_col_service", function() {    	       	 	

   	  $("#service_list").slideToggle("fast");
   });
   $( document ).on( "click", "#filters_col_ratings", function() {    	       	 	

   	  $("#ratings_list").slideToggle("fast");
   });
   ...
   $( document ).on( "click", ".row_del", function() {
              ...
              bootbox.confirm(js_lang.deleteWarning, function(results) {
                  if(results){
                    row_delete( $('.row_del').attr("rev"),$("#tbl").val(), $('.row_del'));  
                  }
              });
    });
...
function isCheckedRestaurent(){
 ...
 $( document ).on( "click", ".cuisine_check", function() {
    
    if($(this).is(':checked')){
        var cu_id = $(this).val();

        var action=$("#action").val();	
        if(cu_id != "previous_all"){
            var params=$("#frm_table_list").serialize()+"&cuisine="+cu_id;
        }else{
	...
	if ( action=="searchArea"){
            ...
            success: function(data){ 
                busy(false); 
                if(data.code == 1){
                    $("#resturant_list_search").html(data.details);
                    $(".countResult").html(data.msg['counts']);
                   /* $("#filter_type_cuisine").html(data.msg['filter_cuisine']);*/
                    $(".allCounts").html("("+data.msg['counts']+")");
                   
                }else{
                      $("#resturant_list_search").html("<p class='text-danger'>Sorry but we cannot find what you are looking for.</p>");
 ...
 $( document ).on( "click", ".icheck_ratings", function() {  
    if($(this).is(':checked')){
        
        var cu_id = $(this).val();
        var action=$("#action").val();	
        var cu_name = $(this).attr("name");

        if(cu_name == "ratings"){
            
            var params=$("#frm_table_list").serialize()+"&ratings="+cu_id; 
            
        }else if(cu_name == "delivery"){
            
            var params=$("#frm_table_list").serialize()+"&delivery="+cu_id;
            
        }else if(cu_name == "distance"){
            
            var params=$("#frm_table_list").serialize()+"&distance="+cu_id;
            
        }else if(cu_name == "minimum_order"){
            
            var params=$("#frm_table_list").serialize()+"&minimum_order="+cu_id;
        }
...
{           
    ...
    } else if( action=="favouriteRestorantList"){
        params+="&action=favouriteRestorantList";
        sEmptyTable=js_lang.tablet_1;
               ...
               "fnInitComplete": function (oSettings, json) {                                                 
                  if ( oSettings._iRecordsTotal<=0){
                     ...
                     //alert(oSettings._iRecordsTotal);
                      $(".countResult").html(oSettings._iRecordsTotal);
...
function maponZoom(){
    ...
    geocoder.geocode( { 'address': currentLocation}, function(results, status) {
          if (status == google.maps.GeocoderStatus.OK) {
           ...
           $("#cureentlang").val(results[0].geometry.location.lng());
           $("#cureentlat").val(results[0].geometry.location.lat());
          }
        });
}
maponZoom();
...
if ( $("#error-message-wrapper").exists() ){
...
var idExistasInForm=document.getElementById("frm_table_list");
if(idExistasInForm){
 // table2();   
}
...
function research_merchant()
{
    ...
    $('input:checkbox.filter_ratings').each(function () {
        var sThisVal = (this.checked ? $(this).val() : "");
        if ( sThisVal !=""){
            filter_ratings+=sThisVal+",";
        }
    });
    ...
    filter_search=$("#search_by_name").val();
    ...
    new_params+="&filter_ratings="+filter_ratings;
    new_params+="&filter_search_name="+filter_search;
...
function uk_msg(msg)
{
	$.UIkit.notify({
	   ...
	   //status:"warning"
        ...
        $("#has_error_message").html("<div class='alert alert-danger'>"+msg+"<div>");
        // $("#has_error_message").html("<div class='alert alert-danger'>"+msg+"<div>");
        setTimeout(function(){ $("#has_error_message").html(""); }, 3000);
}

function uk_msg_sucess_partner(msg){
    $.UIkit.notify({
	   message :msg,
	   timeout :5500,	   
	   status:'success',
    ...
    success: function(data){ 
    	...
    	if (data.code==1){
    		...
    		//$(".section-to-menu-user").append(data.details);
                uk_msg_sucess("Welcome to Bhukkas");
    		$(".top_front_signup").remove();
    		$(".top_front_sigin").remove();
                $(".front-user-display").show();
                $(".show-submenu").html(data.msg+" <i class='icon-down-open-mini'></i>");
...
jQuery(document).ready(function() {
    ...
    new google.maps.LatLng(22.708195, 75.882439),
    new google.maps.LatLng(22.708195, 75.882439));
	...
	/*$("#origin").geocomplete({
       ...
       /*if ( $("#s").exists() ){
    	if (navigator.geolocation) {
	        ...
	        navigator.geolocation.getCurrentPosition(success_geos);
		  
	    } else {
	        dump('Geo Location is not supported');
	   }
        }*/

	/*function success_geos(position) {	
		if ( $("#s").val()!=""){
		 	 return;
		 }	
...
function single_food_item_add(item_id,price,size)
{
	...
	dump("price:"+price);
	dump("size:"+size);*/
   // console.log('single_food_item_add',item_id,price,size);
	var params='';
	params="merchant_id="+$("#merchant_id").val();
...
jQuery(document).ready(function() {		
	...
	$( document ).on( "change", ".s_city", function() {    	
        ...
        $( document ).on( "change", "#citySearchSelect", function() { 
            
	      var selected=$("#citySearchSelect option:selected").text();
              var selected_val=$("#citySearchSelect option:selected").val();
              var selected_category=$("#categorySelect option:selected").val();
              var params="action=setSessionCity&currentController=store&city="+selected+"&cid="+selected_val+"&category="+selected_category+"&check=1";	
              $.ajax({    
                type: "POST",
                url: ajax_url,
                data: params,
                ...
                success: function(data){ 
                    ...
                    if (data.code==1){
                        ...
                        $( "#s" ).autocomplete({
                          source: data.details['location']
                        });

                         if(data.details['city'] == 'gwalior'){
                           $(".pop-bg").css('background-image', 'url(' + sites_url + '/assets/front/img/ad/imgpsh_gwaliors.png)');
                         }else if(data.details['city'] == 'indore'){
                           $(".pop-bg").css('background-image', 'url(' + sites_url + '/assets/front/img/ad/Hotel-section.png)');  
                         }else if(data.details['city'] == 'bhopal'){
                           $(".pop-bg").css('background-image', 'url(' + sites_url + '/assets/front/img/ad/imgpsh_bhopal.png)');  
                         }
                         uk_msg_sucess("welcome your city "+data.msg);
                         
                         //updateAjaxRest();
                         updateAjaxRestFeature();
                          /*window.location.reload();*/    
                    }
                }, 
                error: function(){	        	    	
	      ...
	      var selected=$("#citySearchSelect option:selected").text();
             ...
             // console.log(selected_category);
              ...
              if(typeof selected_val == 'undefined'){
               ...
               if(typeof selected == 'undefined' || selected.length == 0 || selected.length == null){
              ...
              var params="action=setSessionCity&currentController=store&city="+cityName+"&cid="+cid+"&category="+selected_category;;	
              $.ajax({    
                type: "POST",
                url: ajax_url,
                data: params,
...
function getSponseredMerchant(){
    
     var params="action=frontSponseredMerchantList&currentController=store";	
              ...
              $.ajax({    
                ...
                success: function(data){ 
                    busy(false);	    	
                    if (data.code==1){
                       $("#merchant_list_dispaly_sponsered").html(data.details);   
                    }
                }, 
                error: function(){	        	    	
                    busy(false); 
...
setInterval(function(){
    getSponseredMerchant(); 
}, 50000);

function updateAjaxRest(){
     var selected_category=$("#categorySelect option:selected").val();
     var params="action=getFeatureMerchantAjax2&currentController=store&category="+selected_category;	
              ...
              $.ajax({    
                ...
                success: function(data){ 
                    busy(false);	    	
                    if (data.code==1){
                       $("#merchant_list_dispaly").html(data.details);   
                    }else{
                      $("#merchant_list_dispaly").html("");  
                    }
                }, 
                error: function(){	        	    	
                    busy(false); 
...
function updateAjaxRestFeature(){
     var selected_category=$("#categorySelect option:selected").val();
     var params="action=getFeatureMerchantAjax&currentController=store&category="+selected_category;	
              ...
              $.ajax({    
                ...
                success: function(data){ 
                    busy(false);	    	
                    if (data.code==1){
                       $("#merchant_list_dispaly_feature").html(data.details);   
                    }else{
                        $("#merchant_list_dispaly_feature").html("");
                    }
                }, 
                error: function(){	        	    	
                    busy(false); 
    ...
    var $email = $('#email-error');
    $.validate({ 	
    language : jsLanguageValidator,
    form : '#form-email',
    errorMessagePosition: $email,
    errorMessageClass: 'invalid-error',
    scrollToTopOnError : false,
    onError : function() {      
    },
    onSuccess : function() {           
      form_submit('form-email');
      return false;
    }  
});

    $( document ).on( "click", ".fa-like", function() { 
        //$(this).toggleClass('fa-heart');
       // $(this).toggleClass('fa-heart-o');
    });
    
    function addFavorite(m_id,c_id){
        
        if(c_id == '0'){
            bootbox.confirm("Sorry but you need to login to make a favorite", function(result) {
            if(result){
          ...
          bootbox.confirm("Are you sure want to add this restaurant in your favorite list?", function(result) {
            if(result){
                
                 var params="action=addFavoriteMerchant&currentController=store&m_id="+m_id+"&c_id="+c_id;
                $.ajax({    
                    type: "POST",
                    url: ajax_url,
                    data: params,
                    ...
                    success: function(data){ 
                        ...
                        if (data.code==1){
                           uk_msg_sucess(data.msg);
                           $("#like_"+m_id).toggleClass('fa-heart-o');
                           $("#like_"+m_id).toggleClass('fa-heart');
    ...
    function rmFavorite(m_id,c_id){
        
          if(c_id == '0'){
            bootbox.confirm("Sorry but you need to login to remove a favorite", function(result) {
            if(result){
          ...
          bootbox.confirm("Are you sure want to remove this restaurant in your favorite list?", function(result) {
            if(result){
                
                var params="action=removeFavoriteMerchant&currentController=store&m_id="+m_id+"&c_id="+c_id;
                $.ajax({    
                    type: "POST",
                    url: ajax_url,
                    data: params,
...
jQuery(document).ready(function() {
 ...
 $('#register,#login_2,#partnerUs').on('hidden.bs.modal', function (e) {
  $(this)
...
$("#isUserName").click(function(){
    ...
    if($(this).prop("checked") == true){
        ...
        if(email == ""){
            bootbox.alert("Please enter first contact email.", function() {
           });
             $("#isUserName").attr('checked',false); 
        }else{
            $(".user-class").val(email);
            $(".user-class").prop('readonly',true); 
        }
       
    }else{
       $(".user-class").prop('readonly',false);
       $(".user-class").val("");
    }
});
...
$('.user-email').on('keyup',function(){
    ...
    if($("#isUserName").prop("checked") == true){
        var email = $(".user-email").val();
         $(".user-class").val(email);
    }
