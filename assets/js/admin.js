        	...
        	if (data.code==1){
        		...
        		if ( action=="switchMerchantAccount"){
        			window.location.href=data.details;
                        ...
                        if ( action=="addOrderStatus"){
                                setTimeout(function(){ window.location.href=data.details; }, 2000);
                                return;
        		}
                        if ( action=="addCuisine"){
                                setTimeout(function(){ window.location.href=data.details; }, 2000);
                                return;
        		}
                        if ( action=="addVoucherNew"){
                                setTimeout(function(){ window.location.href=data.details; }, 2000);
                                return;
        		}
...
jQuery(document).ready(function() {
	...
	$( document ).on( "click", ".row_del", function() {
        ...
        bootbox.confirm(js_lang.deleteWarning, function(results) {
           if(results){
              row_delete( _this.attr("rev"),$("#tbl").val(), _this);  
           }
        });
    });
...
jQuery(document).ready(function() {	
	if ( $(".j_date_normal").exists()){
        ...
        if ( $(".j_date_normal_commission").exists()){
	    jQuery(".j_date_normal_commission").datepicker({ 
	    	dateFormat: 'yy-mm-dd' ,    	
	        altFormat: "yy-mm-dd",       
	    	changeMonth: true,
	    	changeYear: true ,
...
$('.iCheck-helper').click(function(){
    ...
    var checkDays = [];
    var uncheckDays = [];
    $("input.icheckDays").each(function(index, element){
        if(element.checked){
            isClass = $(this).val();
            if($.inArray(isClass, days) !== -1){
                 checkDays.push(isClass);
                 removeA(days, isClass);
            }
        }
    });
    $.each(checkDays,function(index, value){
      $("."+value).attr("required", "true");   
      console.log("true");
    });
  
    $.each(days,function(indexs, values){
...
function activeCuisine(item_id,status){   
   busy(true);   
   var params="action=activeCuisine&item_id="+item_id+"&status="+status;
   if(status == 0){
    var msg = "You are about to active the selected items. 'Cancel' to stop, 'OK' to delete.?";
   }else{
      var msg = "You are about to inactive the selected items. 'Cancel' to stop, 'OK' to delete.?";  
   }
...
function featuredCuisine(item_id,featured){   
   busy(true);   
   var params="action=featuredCuisine&item_id="+item_id+"&featured="+featured;
   $.ajax({    
   type: "POST",
   url: ajax_url,
   data: params,
