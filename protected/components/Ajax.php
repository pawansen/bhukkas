...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	{					
		...
		if (!empty($this->data['id'])){
			...
			if ( $this->updateData("{{voucher_new}}",$params,'voucher_id',$this->data['id'])){
				$this->code=1;
                                $this->details = Yii::app()->createUrl('admin/voucher');
	    		$this->msg=t("Successfully updated voucher");
			} else $this->msg=t("Failed cannot update records");	    	
		} else {
	        ...
	        if ( $this->insertData('{{voucher_new}}',$params)){
	        	//$this->details=Yii::app()->db->getLastInsertID();
                        $this->details = Yii::app()->createUrl('admin/voucher');
	    		$this->code=1;
	    		$this->msg=Yii::t("default","Successfully added voucher");		 
	        } else $this->msg=t("ERROR: Something went wrong");		
		}
	}
	...
	public function VoucherListNew()
     ...
     {     	
 	    $slug=createUrl("store/myreviews");
		$stmt="SELECT a.*,
                 (SELECT restaurant_name from  {{merchant}} WHERE merchant_id = a.merchant_id) as name
                  FROM
		{{review}} a
		WHERE
		...
		if ($res=$this->rst($stmt)){
		   ...
		   foreach ($res as $val) {
                                $rating= ($val['rating'] != '') ? round($val['rating']) : 0 ;
                                $rating_star_user= ""; 
                                for($i=1; $i<=5;$i++){
                                    $rating_star_user.="<span class='rating'>";
                                    if($i <= $rating){ 
                                      $rating_star_user.="<i class='icon_star voted'></i> ";
                                     }else{
                                       $rating_star_user.=" <i class='icon_star invoted'></i> ";
                                     }
                                     $rating_star_user.="</span>";
                                }
                                $action = "<div class=\"options\">
	    		<a href=\"reviews/$val[merchant_id]\" >".ucwords($val['name'])."</a>
	    		</div>";
                                
		   	   $feed_data['aaData'][]=array(
		   	      $action,
		   	      $rating_star_user,
		   	      substr($val['review'],0,100),
     ...
     {     	
		...
		if ($res=$this->rst($stmt)){
		   foreach ($res as $val) {				   	    			   	    
				$action="<div class=\"options\">
	    		<a href=\"$slug/do/add/id/$val[id]\" >".Yii::t("default","<i class='fa fa-edit'></i>")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[id]\" >".Yii::t("default","<i class='fa fa-trash'></i>")."</a>
	    		</div>";		   	   
		   	   $feed_data['aaData'][]=array(
		   	      ucwords($val['address']),
		   	      ucwords($val['location_name']),
     ...
     {     	
     	...
     	if ( isset($this->data['id'])){
     		...
     		if ( $this->updateData("{{address_book}}",$params,'id',$this->data['id'])){
     			$this->code=1;
     			$this->msg=Yii::t("default","Your address successfully updated");		 
     		} else $this->msg=t("ERROR: Something went wrong");	
     	} else {
     	    if ( $this->insertData('{{address_book}}',$params)){
	        	$this->details=Yii::app()->db->getLastInsertID();
	    		$this->code=1;
	    		$this->msg=Yii::t("default","Your address successfully added");		 
	        } else $this->msg=t("ERROR: Something went wrong");		
     	}
	...
	{		
        ...
        public function favouriteRestorantList(){
            $client_id = Yii::app()->functions->getClientId();
          ...
          $stmt = "SELECT c.*,a.*
            ...
            if ($res = $this->rst($stmt)) {
                foreach ($res as $val) {
                    $action = "
	    		<a href=\"reviews/$val[merchant_id]\" >".ucwords($val['restaurant_name'])."</a>
