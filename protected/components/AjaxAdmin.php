...
if (!class_exists('AjaxAdmin')) {
    ...
    class AjaxAdmin extends DbExt {
        ...
        public function login() {
            ...
            if ($res = $DbExt->rst($stmt)) {
                ...
                $this->msg = Yii::t("default", "You have successfully logged in");
            } else
        ...
        public function addMerchant() {
            ...
            $params['merchant_category'] = $this->data['category'];
            ...
            $params['bank_name'] = isset($this->data['bank_name']) ? $this->data['bank_name'] : '';
            $params['account_number'] = isset($this->data['account_number']) ? $this->data['account_number'] : '';
            $params['branch'] = isset($this->data['branch']) ? $this->data['branch'] : '';
            $params['ifsc_code'] = isset($this->data['ifsc_code']) ? $this->data['ifsc_code'] : '';
  ...
  public function emailsubscription(){
                $subject = Yii::t("default", "Email Subscription");
                $admin_to = yii::app()->functions->getOptionAdmin('alert_email_receiver'); 
                $user_to = $this->data['email_address'];
               
                $from = 'info@bhukkas.com';
                if (empty($admin_to) && empty($user_to)) {
            ...
            $params = array(
                ...
                $obj = new Widgets();
                $tpluser = $obj->emailTemplatesEmailSuscription();
                $tpladmin  = $obj->emailTemplatesadmin($this->data);
                $title = "Bhukkas";
            if ($this->insertData("{{newsletter}}", $params)) {
//                $this->code = 1;
//                $this->msg = Yii::t("default", "Thank you for your submission");
                 if (Yii::app()->functions->sendEmail($admin_to, $from, $subject, $tpladmin, $title)) {
                      if(Yii::app()->functions->sendEmail($user_to, $from, $subject, $tpluser, $title)){
                        $this->code = 1;
                        $this->msg = Yii::t("default", "Thank you for your submission");
                          }
                    } else
        ...
        public function addCategory() {
            ...
            if (isset($this->data['id']) && is_numeric($this->data['id'])) {
                ...
                if ($res) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", 'Record has been updated successfully');
                } else
                    $this->msg = Yii::t("default", "ERROR: cannot update");
            } else {
                if ($res = $command->insert('{{category}}', $params)) {
                    $this->details = Yii::app()->db->getLastInsertID();
                    $this->code = 1;
                    $this->msg = Yii::t("default", 'Record has been saved successfully');
                } else
        ...
        public function UpdateMerchant() {
            ...
            $params['state'] = isset($this->data['state']) ? $this->data['state'] : '';
            $params['abn'] = isset($this->data['abn']) ? $this->data['abn'] : '';
            $params['merchant_category'] = isset($this->data['category']) ? $this->data['category'] : '';
            $params['bank_name'] = isset($this->data['bank_name']) ? $this->data['bank_name'] : '';
            $params['account_number'] = isset($this->data['account_number']) ? $this->data['account_number'] : '';
            $params['branch'] = isset($this->data['branch']) ? $this->data['branch'] : '';
            $params['ifsc_code'] = isset($this->data['ifsc_code']) ? $this->data['ifsc_code'] : '';
            ...
            if ($res) {
                $this->code = 1;
                $this->msg = Yii::t("default", 'Merchant has been updated successfully');
            } else
                $this->msg = Yii::t("default", "ERROR: cannot update");
        }
        ...
        public function merchantSettings() {
            ...
            $this->code = 1;
            $this->msg = Yii::t("default", "Setting has been saved successfully");
        }
        ...
        public function setSessionCity() {
            ...
            $check = $_POST['check'];
            // if(!empty(Yii::app()->session['category']) && isset(Yii::app()->session['category'])){
            if (!empty($check) && $check == 1) {
                Yii::app()->session['category'] = $_POST['category'];
            }
            // }
            if (!empty($cid)):
                $this->msg = ucwords($_POST['city']);
                Yii::app()->session['cid'] = $cid;
                Yii::app()->session['myCity'] = $_POST['city'];
                $this->details['city'] = strtolower($_POST['city']);
                $rs = Yii::app()->functions->getLocationAreaByCity($cid);
                $location = array();
                foreach ($rs as $rows):
       ...
       public function frontSponseredMerchantList() {
        $sponsered = array();  
        $stmt = "SELECT merchant_id,restaurant_slug,restaurant_name,sponsored_image FROM
		      {{view_merchant}} where is_sponsored = 2 ORDER BY RAND() LIMIT 4 OFFSET 0
		";
        if ($res = $this->rst($stmt)) {
            
            foreach ($res as $rest):
                  $img = ($rest['sponsored_image']) ? uploadURL().'/'.$rest['sponsored_image'] : uploadURL().'/100X100.png';
                  $url = Yii::app()->CreateUrl('/store/menu/merchant/'.$rest['restaurant_slug']);
                  $name = $rest['restaurant_name'];
                  $sponsered [] = "<aside class='col-md-3 m-t-xs-10 col-sm-6'>
                            <a href='".$url."' title='".$name."'> <img src='".$img."' class='img-responsive'></a>
                        </aside>";
                
            endforeach;
            
            $this->details = $sponsered; 
            $this->code = 1;
        ...
        public function getFeatureMerchantAjax2() {
            $category = $_POST['category'];
            $restaurant = array();
            $db_ext = new DbExt;
            $and = "";
            ...
            if ($category != 'all') {
                $and.= "AND merchant_category = '" . $category . "' ";
            }
            $stmt = "SELECT a.*,  
    	    (
	    	select option_value
			...
			ORDER BY restaurant_name DESC LIMIT 0,7
		";
            //is_sponsored='2'
            $upload_url = Yii::app()->request->baseUrl . "/upload";
            $assets = Yii::app()->request->baseUrl . "/assets";
            $base = Yii::app()->request->baseUrl . "/store/menu/merchant/";
                ...
                foreach ($res as $val) {
                    if ($counts <= 4) {
                        ...
                        if (empty($merchant_logo)) {
                            $merchant_logo = $assets . "/images/thumbnail-mini.png";
                        } else {
                            $merchant_logo = $upload_url . "/$merchant_logo";
                        }
                        $restaurant [] = " <figcaption class='col-md-3 col-sm-3'>
                            <aside class='pop-hotel'>
                                <a href='" . $base . "" . $val['restaurant_slug'] . "'>
                               <div class='thumbnail'>
                                    <div class='caption'>
                                        <h4> " . ucwords($val['restaurant_name']) . "</h4>
                                    </div>
                                    <img src='" . $merchant_logo . "' alt='' class='img-responsive'>
                                </div>
                               </a>
                            </aside>
                       </figcaption>";
        ...
        public function getFeatureMerchantAjax() {
            $category = $_POST['category'];
            // if(!empty(Yii::app()->session['category']) && isset(Yii::app()->session['category'])){
		...
		";
            ...
            $upload_url = Yii::app()->request->baseUrl . "/upload";
            $assets = Yii::app()->request->baseUrl . "/assets";
            $base = Yii::app()->request->baseUrl . "/store/reviews/";
            // $merchant_logo_more = Yii::app()->request->baseUrl."/assets/front/img/ad/Hotel8.png";
            $create_url_all = Yii::app()->request->baseUrl . "/store/searchArea?cu=all";
            if ($res = $db_ext->rst($stmt)) {
                ...
                $restaurant [] = " <figcaption class='col-md-3 col-sm-3'>
                            <aside class='pop-hotel'>
                                <a href='" . $create_url_all . "'>
                               <div class='thumbnail'>
                                    <div class='caption'>
                                        <h4> More </h4>
                                    </div>
                     <img src='" . Yii::app()->request->baseUrl . "/assets/front/img/ad/Hotel8.png' alt='' class='img-responsive'>
                                </div>
                               </a>
                            </aside>
                       </figcaption>";
        ...
        public function getRestaurentByMap() {
            ...
            if (isset(Yii::app()->session['myCity'])) {
                ...
                $currentCity = Yii::app()->session['myCity'];
                $and = "WHERE  city LIKE '%" . $currentCity . "%'";

                if (Yii::app()->session['rest_all'] != 'all') {
                    if (isset(Yii::app()->session['street_address'])) {

                        $street_address = Yii::app()->session['street_address'];
                        $and.= " AND  street LIKE '%" . $street_address . "%'";
                    }
                } else {
            ...
            if (isset(Yii::app()->session['category']) && Yii::app()->session['category'] != 'all') {
                $categorys = Yii::app()->session['category'];
                $and.= " AND  merchant_category = '" . $categorys . "'";
            }



            if (isset($_POST['merchant_id_map']) && !empty($_POST['merchant_id_map'])) {

                $merchant_id_map = $_POST['merchant_id_map'];
                $and.= " AND  merchant_id = " . $merchant_id_map . "";
            }

            $stmt = "SELECT a.restaurant_name,a.street,a.city,a.latitude,a.lontitude,a.cuisine,a.restaurant_slug,  
                (
                 select option_value
                 from 
                 {{option}}
            ...
            $allArray = array();
            $orderNowUrl = baseUrl() . "/store/menu/merchant/";
            $pin = array('American', 'Chinese', 'Fish', 'Mexican', 'Pizza');
            if ($rs = $DbExt->rst($stmt)) {
                    ...
                    if ($cuisine != false) {
                        foreach ($cuisine as $valc) {
                            if (array_key_exists($valc, (array) $cuisine_list)) {
                                ...
                                $exists_cuisine [] = strtolower($cuisine_list[$valc]);
                                $exists_cuisine_keys [strtolower($cuisine_list[$valc])] = $valc;
                            }
                        }
                        $resto_cuisine = !empty($resto_cuisine) ? substr($resto_cuisine, 0, -2) : '';
                    }
                    $logo = $rows ['merchant_logo'];

                    if (empty($logo)) {
                        $logo = assetsURL() . "/images/thumbnail-mini.png";
                    } else
                        $logo = uploadURL() . "/$logo";

                    $mapArray ['type_point'] = $resto_cuisine;
                    $mapArray ['name'] = $rows ['restaurant_name'];
                    $mapArray ['description_point'] = ucwords($rows ['street'] . " " . $rows ['city']);
                    $mapArray ['location_latitude'] = $rows ['latitude'];
                    $mapArray ['location_longitude'] = $rows ['lontitude'];
                    $mapArray ['map_image_url'] = $logo;
                    $mapArray ['name_point'] = $rows ['restaurant_name'];
                    $mapArray ['url_point'] = $orderNowUrl . $rows ['restaurant_slug'];
                    $allArray [$i] = [$mapArray];
                ...
                $this->code = 1;
                $this->details = $allArray;

                $this->msg = Yii::t("default", "Succssful");
            } else
                $this->msg = Yii::t("default", "ERROR: Failed map data.");
        }

        public function searchArea1() {

            //unset( $_SESSION['total_records_found'] ); 
            $resto_cuisine = '';
            $rating = '';
            $minimum = '';
            $pay_by = '';
            ...
            $exists_cuisine = array();
            $exists_cuisine_keys = array();
            ...
            if (is_array($search_str) && count($search_str) >= 2) {
                ...
                $street_address = isset($search_str[0]) ? trim($search_str[0]) : '';
            } else {
                $city = trim($this->data['s']);
                $state = trim($this->data['s']);
            }
            $city = $this->data['city'];
            if ($res = Yii::app()->functions->searchByArea($city, $state, $street_address)) {
                $this->code = 1;
                $this->details = '';
                $ict = 0;
                ...
                foreach ($res as $rows) {
                    ...
                    $serviceName = '';
                    if ($service == 1):
                        $serviceName = "<ul> <li>Take away<i class='icon_check_alt2 ok'></i></li>
                  <li>Delivery<i class='icon_check_alt2 no'></i></li> </ul>";
                    elseif ($service == 2):
                        $serviceName = "<ul><li>Take away<i class='icon_check_alt2 ok'></i> </ul>";
                    elseif ($service == 3):
                        $serviceName = "<ul><li>Delivery<i class='icon_check_alt2 no'></i></li> </ul>";
                    endif;

                    $restaurant [] = " <div class='strip_list wow fadeIn' data-wow-delay='0." . $ict . "s'>
           
              <div class='row'>
               <div class='col-md-9 col-sm-9'>
                <div class='desc'>
                 <div class='thumb_strip'>
                  <a href='#'><img src='" . $merchant_logo . "' alt=''></a>
                 </div>
                 <div class='rating'>
                 " . $rating_star . "
                 </div>
                 <h3>" . ucwords($rows['restaurant_name']) . "</h3>
                 <div class='type'>
                  " . $resto_cuisine . "
                 </div>
                 <div class='location'>
                  " . ucwords($rows['street']) . ", " . ucwords($rows['city']) . ", " . ucwords($rows['state']) . " </br>Minimum order:" . ucwords($rows['minimum_order']) . ".00 INR
                </br> Delivery Fee :" . $rows['delivery_charges'] . ".00 INR</div>
               ...
               <div class='col-md-3 col-sm-3'>
                <div class='go_to'>
                 <div>
                  <a href='" . $orderNowUrl . $rows['restaurant_slug'] . "' class='btn_1'>Order Now</a>
                 </div>
                </div>
               </div>
              </div> 
             </div>";
                    //$_SESSION['total_records_found'] = $ict;
                }
            }else {
                $this->code = 0;
                //$_SESSION['total_records_found'] = 0;
            }
            if ($records = Yii::app()->functions->searchByRecordsCount($city, $state, $street_address)) {
                $this->msg['counts'] = $records;
            } else {
                $this->msg['counts'] = 0;
            }
            $this->details = 1;
            $this->details = $restaurant;
            $this->msg['filter_cuisine'] = '';
            /* if(!empty($exists_cuisine)){
              $commonCounts = array_count_values($exists_cuisine);
              $filter_cuisine = "<h6>Type</h6><ul>";
              $filter_cuisine .="<li><label><input type='checkbox' value='previous_all' class='cuisine_check'>  All <small class='allCounts'></small></label></li>";
              $colorCounts = 1;
              foreach($commonCounts as $k=>$values){
              $filter_cuisine .="<li><label><input type='checkbox' value='$exists_cuisine_keys[$k]' class='cuisine_check'>  ".  ucwords($k)." <small>(".$values.")</small></label><i class='color_".$colorCounts."'></i></li>";
              $colorCounts++;
              if($colorCounts == 7){
              $colorCounts = 1;
              }
            ...
            /* echo $filter_cuisine;
              print_r($exists_cuisine_keys);
              exit(); */
        }

        public function countResultSearchRecords() {

            if ($res = Yii::app()->functions->searchByArea($city, $state)) {

                $this->details = Yii::app()->functions->search_result_total;
            } else {
                $this->details = 0;
            }
            $this->code = 1;
        ...
        public function searchArea() {
            ...
            if (is_array($search_str) && count($search_str) >= 2) {
                $location = isset($search_str[1]) ? trim($search_str[1]) : '';
                $state = isset($search_str[2]) ? trim($search_str[2]) : '';
            } else {
                $location = trim($this->data['s']);
                $state = trim($this->data['s']);
            }
            ...
            if ($res = Yii::app()->functions->searchByArea($city, $state)) {
                ...
                if (is_array($res) && count($res) >= 1) {
                    ...
                    if ($total > 0) {
                        $_SESSION['totalOtable'] = $total;
                    } else {
                        $_SESSION['totalOtable'] = 0;
                    }
                    ...
                    foreach ($res as $val) {
                        ...
                        $rating_star = '';
                        $ratingT = ($val['ratings'] != '') ? round($val['ratings']) : 0;
                        for ($i = 1; $i <= 5; $i++) {
                            if ($i <= $ratingT) {
                                $rating_star.="<i class='icon_star voted'></i>";
                            } else {
                        ...
                        if ($val['is_sponsored'] == 2) {
                            $is_sponsored = "<span class='btn-sponser'>" . Yii::t("default", "sponsored") . "</span>";
                        }
                        ...
                        if ($is_merchant_open == TRUE) {
                            $tag_open = "<span class='btn-open'>" . t("Open") . "</span>";
                        } else {
                            if ($merchant_preorder) {
                                $tag_open = "<span class='btn-pre-order'>" . t("Pre-Order") . "</span>";
                            } else {
                                $tag_open = "<span class='btn-closed'>" . t("Closed") . "</span>";
                                $now_open = Yii::app()->functions->isMerchantOpenNowTime($val['merchant_id']);
                            }
                        }
                        ...
                        $resto_info = "" . $is_sponsored . "<h3><a href=\"" . baseUrl() . "/store/menu/merchant/" . $val['restaurant_slug'] . "\">" .
                                ucwords($val['restaurant_name']) . "</a></h3>";
                        ...
                        if (array_key_exists($val['country_code'], (array) $country_list)) {
                            $countryName = $country_list[$val['country_code']];
                        }


                        $resto_info.="<h5>" . $resto_cuisine . "</h5>";


                        $resto_info.="<div class='rating'>" . $rating_star . "</div><ul class=\"rest-detail\">";



                        /* $resto_info.="<li>".$val['street']." ".$val['city'] ." ".$val['post_code']." ". $val['service'] . "</li>"; */
                        ...
                        if (!empty($from_address)):
                            if ($ft == TRUE) {
                                $resto_info.="<li><label>" . Yii::t("default", "Distance") . "</label>  $miles_raw </li>";
                                ...
                                if ($merchant_distance_type == "km") {
                                   // $distance_type = Yii::t("default", "km");
                                   // $resto_info.="<li><label>" . Yii::t("default", "Distance") . "</label> $kms " . Yii::t("default", "km") . "</li>";
                                   // $distancesya = $kms;
                                } else {
                                    //$resto_info.="<li><label>" . Yii::t("default", "Distance") . "</label> $miles_raw " . Yii::t("default", "miles") . "</li>";
                                   // $distance_type = Yii::t("default", "miles");
                                }
                            }
                        endif;
                        ...
                        if (is_numeric($val['distance'])) {
                             $distance_t = number_format((float) $val['distance'], 1, '.', '');
                            $resto_info.="<li><label>" . Yii::t("default", "Distance") . "</label> " . $distance_t . " " . $distance_type . "</li>";
                        }
                        
                        if ($delivery_est != '') {
                            $resto_info.="<li><label>" . Yii::t("default", "Delivery Est") . "</label> " . $delivery_est . "</li>";
                        }

                        if (is_numeric($mt_delivery_miles)) {
                            $resto_info.="<li><label>" . Yii::t("default", "Delivery In") . "</label> " . $mt_delivery_miles . " " . $distance_type . "</li>";
                        }
                        ...
                        if (is_numeric($delivery_fee) && $delivery_fee >= 1) {
                            $resto_info.="<li><label>" . Yii::t("default", "Delivery Fee") . "</label> " .
                                    displayPrice(getCurrencyCode(), prettyFormat($delivery_fee)) . "</li>";
                        } else {
                            $resto_info.="<li><label>" . Yii::t("default", "Delivery Fee") . "</label> " .
                                    " " . Yii::t("default", "Free Delivery") . "" . "</li>";
                        }
                        ...
                        if (is_numeric($client_id)):
                          $my_fav = Yii::app()->functions->getFavoriteList($client_id,$val['merchant_id']);  
                          $client = $client_id; 
                        endif;
                        $ofHtml = "";
                        if ($my_fav == '') {
                            $ofHtml = "<span class='btn-offer'>" . $offers . "<a  onclick=\"addFavorite($val[merchant_id],$client)\" title=\"Favorited By\" class=\"btn-fav\"><i id=\"like_$val[merchant_id]\" class=\"fa fa-like fa-heart-o\"></i></a></span>";
                        }else{
                           $ofHtml = "<span class='btn-offer'>" . $offers . "<a  onclick=\"rmFavorite($val[merchant_id],$client)\" title=\"Favorited By\" class=\"btn-fav\"><i id=\"like_$val[merchant_id]\" class=\"fa fa-like fa-heart\"></i></a></span>"; 
                        }
                        
                        $resto_btn = "" . $ofHtml . " <div>  
						<a class=\"btn_1\" href=\"" . baseUrl() . "/store/menu/merchant/" . $val['restaurant_slug'] . "\">";
                        $resto_btn.=Yii::t("default", "GO to menu");
                        $resto_btn.="</a></div>";

                        $resto_btn.="<div class=\"spacer\"></div>";
                        ...
                        if ($min_orde != '') {
                            ...
                            if (!empty($now_open)) {
                                ...
                                $resto_info.="<li><span class='btn-closed'><i class='fa fa-clock-o'><i> " . $now_open . "</span></li>";
                            }
                        }
                            ...
                            if ($val['item_id'] == $item['item_id']) {
                                ...
                                if (is_array($item['addon_ids']) && count($item['addon_ids']) >= 1) {
                                    if (count($val['addon_ids']) >= count($item['addon_ids'])) {
                                        ...
                                        if (count($compare) <= 0) {
                                            if ($cooking_ref == TRUE && $ingredients == TRUE && $item_size == TRUE) {
                                                $found = true;
                                                break;
                                            }
                                        }
        ...
        public function setDeliveryOptions() {
            ...
            if (!empty($this->data['delivery_date'])) {
                    $this->data['delivery_date'] = date("Y-m-d", strtotime($this->data['delivery_date']));
            }
        ...
        public function addFavoriteMerchant(){
             $m_id = $this->data['m_id'];
             $c_id = $this->data['c_id'];
             $params =  array(
                 'merchant_id' => $this->data['m_id'],
                 'client_id' => $this->data['c_id'],
                 'date_created' => date('c')
             );

             if(Yii::app()->functions->getFavoriteList($c_id,$m_id)){
                $this->msg = Yii::t("default", "already added in your favorite list");   
             }else{
               if($this->insertData("{{favourite}}", $params)){
                $this->code = 1;
                $this->msg = Yii::t("default", "Successfully added in your favorite list"); 
               }else{
                 $this->msg = Yii::t("default", "Failed to added in your favorite list");
               } 
             }
        ...
        public function removeFavoriteMerchant(){
            $m_id = $this->data['m_id'];
            $c_id = $this->data['c_id'];
                    
             $sql = "DELETE FROM {{favourite}} WHERE merchant_id = '$m_id' AND client_id = '$c_id' ";
            
             if($this->qry($sql)){
                $this->code = 1;
                $this->msg = Yii::t("default", "Successfully remove in your favorite list"); 
             }else{
                 $this->msg = Yii::t("default", "Failed to remove in your favorite list");
             }
        }
        ...
        public function addCreditCard() {
            ...
            if (!Yii::app()->functions->getCCbyCard($this->data['credit_card_number'], $cid)) {
                ...
                if ($this->insertData("{{client_cc}}", $params)) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Credit Card has been saved successfully");
                } else
                        ...
                        if (Yii::app()->functions->getOptionAdmin('website_enabled_map_address') == 2) {
                            if (isset($this->data['map_address_toogle'])) {
                                if ($this->data['map_address_toogle'] == 2) {
                                    $geo_res = geoCoding($this->data['map_address_lat'], $this->data['map_address_lng']);
                                    if ($geo_res) {
                                        //dump($geo_res);
            ...
            if (Yii::app()->functions->isClientLogin()) {
                ...
                if (!$res = Yii::app()->functions->isClientRatingExist($this->data['merchant_id'], $client_id)) {
                    ...
                    if ($DbExt->updateData("{{rating}}", $update, 'id', $rating_id)) {
                        $this->code = 1;
                        $this->msg = Yii::t("default", "Rating has been updated successfully");
                    } else {
        ...
        public function loadTopMenu() {
            ...
            if (Yii::app()->functions->isClientLogin()):
                ...
                $this->code = 1;
                /* $this->details=$forms; */
                $this->msg = ucwords(Yii::app()->functions->getClientName());
            else :
                $this->code = 2;
            endif;
        }
        ...
        public function editRatingsReview() {
            if (Yii::app()->functions->isClientLogin()) {
                $client_id = Yii::app()->functions->getClientId();
                $rs = Yii::app()->functions->getReviewRatings($this->data['merchant_id'], $client_id);
                $this->code = 1;
                $this->msg = "OK";
                $this->details = $rs; //json_encode($rs);
            } else
                $this->msg = Yii::t("default", "Cannot remove ratings user is not login.");
        }
        ...
        public function editReviewsRating() {
            ...
            if (Yii::app()->functions->isClientLogin()) {
                ...
                if ($DbExt->updateData("{{review}}", $params, 'id', $this->data['reviewId'])) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Your review has been Update.");
        ...
        public function adminSettings() {
            ...
            Yii::app()->functions->updateOptionAdmin("admin_zipcode_searchtype", isset($this->data['admin_zipcode_searchtype']) ? $this->data['admin_zipcode_searchtype'] : '');
             ...
             Yii::app()->functions->updateOptionAdmin("alert_email_receiver", isset($this->data['alert_email_receiver']) ? $this->data['alert_email_receiver'] : '');

            $this->code = 1;
            $this->msg = Yii::t("default", "Setting saved");
        }
        
        public function merchantAccountInfo(){
            ...
            if($this->updateData("{{merchant}}", $params, 'merchant_id',$this->data['merchant_id'])){
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Your merchant registration is successfull");
                    $this->details = $token;
            }else{
        ...
        public function sponsoreMerchantAdd() {
            if (isset($this->data['merchant_id'])) {
                $params = array(
                    ...
                    'sponsored_image' => ($this->data['spicydish']) ? $this->data['spicydish']: ''
                );
                if ($this->updateData("{{merchant}}", $params, 'merchant_id', $this->data['merchant_id'])) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Successfully updated sponsored merchant");
            ...
            if ($res = $this->rst($stmt)) {
                foreach ($res as $val) {
                    ...
                    $feed_data['aaData'][] = array(
                        ...
                        '<img class="uk-thumbnail uk-thumbnail-mini" src="' . uploadURL() . "/" . $img . '">',
                         $date,
                    );
                }
                $this->otableOutput($feed_data);
            ...
            if ($res = $this->rst($stmt)) {
                ...
                foreach ($res as $val) {
                    ...
                    if ($featured == 1) {
                        ...
                        $feature = "<input type=\"checkbox\" onclick=\"featuredCuisine($val[cuisine_id],$featured)\" >";
                    }
                    $stats = $val['status'];
                    if ($stats == 1) {
                        $status = "<a onclick=\"activeCuisine($val[cuisine_id],$stats)\" class='uk-text-success' href=\"javascript:void(0)\" >" . Yii::t("default", "Active") . "</a>";
                    } else {
                        $status = "<a onclick=\"activeCuisine($val[cuisine_id],$stats)\" class='uk-text-danger' href=\"javascript:void(0)\" >" . Yii::t("default", "Inactive") . "</a>";
                    }
                    $img = ($val['image']) ? $val['image'] : '100X100.png';
                    $feed_data['aaData'][] = array(
                        $i,
        ...
        public function addCuisine() {
            ...
            if ($Validator->validate()) {
                $params = array(
                    ...
                    'image' => isset($this->data['spicydish']) ? $this->data['spicydish'] : '',
                    'featured' => isset($this->data['featured']) ? $this->data['featured'] : '',
                    'status' => $this->data['status'],
                    'date_created' => date('c'),
                    'ip_address' => $_SERVER['REMOTE_ADDR']
                );
        ...
        public function activeCuisine() {
            $cuisine_id = $_POST['item_id'];
            $status = $_POST['status'];

            if ($cuisine_id != '' && $status != '') {
                //Group
                if ($status == 0) {
                ...
                $update = $this->updateData('{{cuisine}}', $update_data, 'cuisine_id', $cuisine_id);

                if ($update) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Status updated successfully.");
                } else {
                    $this->msg = Yii::t("default", "Status not updated.");
                }
            } else {
                $this->msg = Yii::t("default", "Status not updated.");
            }
        ...
        public function featuredCuisine() {
            $cuisine_id = $_POST['item_id'];
            $featured = $_POST['featured'];

            if ($cuisine_id != '' && $featured != '') {
                //Group
                if ($featured == 0) {
                    $update_data['featured'] = 1;
                } else {
                ...
                if ($update) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", "Feature updated successfully.");
                } else {
                    $this->msg = Yii::t("default", "Feature not updated.");
                }
            } else {
                $this->msg = Yii::t("default", "Feature not updated.");
            }
       ...
       /* public function addCuisine() {
        ...
        public function updateClientProfile() {
            ...
            if ($this->updateData("{{client}}", $params, 'client_id', $client_id)) {
                $this->code = 1;
                $this->msg = Yii::t("default", "Your profile successfully updated.");
            } else
        ...
        public function contacUsSubmit() {
            ...
            if ($validator->validate()) {
                ...
                $params = array(
                    ...
                    'contact_country' => $this->data['country'],
                    'contact_message' => $this->data['message']
                );
                $obj = new Widgets();
                $tpl = $obj->emailTemplatesContact($this->data);
//                                echo $tpl;
//                                exit;
                $title = "Bhukkas";
                if ($this->insertData("{{contact_us}}", $params)) {
                    if (Yii::app()->functions->sendEmail($to, $from, $subject, $tpl, $title)) {
                        $this->code = 1;
            ...
            if ($Validator->validate()) {
                if ($res = yii::app()->functions->isClientExist($this->data['username-email'])) {
                    ...
                    if ($this->updateData("{{client}}", $params, 'client_id', $res['client_id'])) {
                        ...
                        if (!sendEmail($to, $sender, Yii::t("default", "Forgot Password"), $tpl,'forgot password')) {
                            $this->details = "failed";
                        } else
        ...
        public function resendActivationCode() {
            ...
            if ($res = $this->rst($stmt)) {
                ...
                $sender = Yii::app()->functions->getOptionAdmin('website_contact_email');
                $to = $res[0]['contact_email'];
                if (!sendEmail($to, $sender, Yii::t("default", "Merchant Registration"), $tpl,'Merchant Registration')) {
                    $this->details = "failed";
                } else
                    $this->details = "ok mail";
                $this->code = 1;
        ...
        public function adminCCAvenue() {
            Yii::app()->functions->updateOptionAdmin("admin_ccAvenue_enabled", isset($this->data['admin_ccAvenue_enabled']) ? $this->data['admin_ccAvenue_enabled'] : '');

            Yii::app()->functions->updateOptionAdmin("admin_ccAvenue_mode", isset($this->data['admin_ccAvenue_mode']) ? $this->data['admin_ccAvenue_mode'] : '');

            Yii::app()->functions->updateOptionAdmin("admin_ccAvenue_key", isset($this->data['admin_ccAvenue_key']) ? $this->data['admin_ccAvenue_key'] : '');

            Yii::app()->functions->updateOptionAdmin("admin_ccAvenue_accesscode", isset($this->data['admin_ccAvenue_accesscode']) ? $this->data['admin_ccAvenue_accesscode'] : '');
            
            Yii::app()->functions->updateOptionAdmin("admin_ccAvenue_workingkey", isset($this->data['admin_ccAvenue_workingkey']) ? $this->data['admin_ccAvenue_workingkey'] : '');
        ...
        public function bookATableMerchant() {
            ...
            if (isset($this->data['send_email'])) {
                if ($this->code == 1 || $this->data['send_email'] == 1) {
                    ...
                    if (!sendEmail($to, $sender, $subject, $template,'Booking Table')) {
                        $this->details = 2;
                    } else
                        $this->details = 1;
                }
        ...
        public function merchantContactList() {
        ...
        public function userContactList() {
            ...
            if ($res = $this->rst($stmt)) {
                foreach ($res as $val) {
                    $date = FormatDateTime($val['contact_create_date']);
                    $feed_data['aaData'][] = array(
                        $i,
                        $val['contact_name'],
                        $val['contact_email'],
            ...
            if (isset($this->data['email_address'])) {
                if ($res = yii::app()->functions->isAdminExist($this->data['email_address'])) {
                    ...
                    if ($this->updateData("{{admin_user}}", $params, 'admin_id', $res[0]['admin_id'])) {
                        ...
                        if (!sendEmail($to, $sender, t("Admin Forgot Password"), $tpl,'Forgot password')) {
        ...
        public function partnerRegistration() {
            ...
            if ($this->insertData("{{partner}}", $params)) {
                $this->details = Yii::app()->db->getLastInsertID();
                $this->code = 1;
                $this->msg = Yii::t("default", "Thank you for your submission. Our restaurant sign-up team will get in touch with you soon. Looking forward to having you in Bhukkas!");
            }
        }
        ...
        public function addCms() {
            if (!empty($this->data['id'])) {
                ...
                $params['date_modified'] = date('c');
                $res = $this->updateData('{{cms}}', $params, 'cms_id', $this->data['id']);
                if ($res) {
                    $this->code = 1;
                    $this->msg = Yii::t("default", 'Cms updated.');
                } else
                    $this->msg = Yii::t("default", "ERROR: cannot update");
            } else {
                $title = $this->data['title'];
                $code = str_replace(' ', '_', trim($title));
                $stmt = "SELECT cms_code FROM
			       {{cms}}
			       WHERE
			       cms_code=" . Yii::app()->db->quoteValue($code);
                $connection = Yii::app()->db;
                $rows = $connection->createCommand($stmt)->queryAll();
                if (count($rows) >= 1) {
                    $this->msg = Yii::t("default", "This title already exist!! Please enter another Title.");
                } else {
                    $params = array(
                        'title' => $this->data['title'],
                        'description' => $this->data['description'],
                    ...
                    if ($this->insertData("{{cms}}", $params)) {
                        $this->code = 1;
                        $this->msg = Yii::t("default", "Data inserted successfully");
                    } else {
                        $this->msg = Yii::t("default", "Oops!! Error occured while inserting data.");
                    }
                }
            }
        }
        
        public function emailSuscriptionList() {
