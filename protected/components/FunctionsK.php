...
{	
   ...
   public function getSMSPackagesById($package_id='')
    ...
    public function reCheckDelivery($data='',$mtid='')
    {
      ...
      public function reCheckDeliveryApi($data='',$mtid='')
    {
    	...
    	if (is_numeric($mt_delivery_miles) || $shipping_enabled==2){
	    	...
	    	$delivery_address = $data['client_info']['address'];	    	
    ...
    public function CheckCustomerMobile($mobile_no='',$client_id='')
    {
       ...
       public function CheckCustomerMobileExists($mobile_no='',$client_id='')
    {
    	$and="";
    	if (!empty($client_id)){
    ...
    {    	
    	...
    	if (!empty($subject) && !empty($client_email)){
    		$tpl=smarty('client_name',$data['first_name'],$tpl);
    		$tpl=smarty('website_name',getOptionA('website_title'),$tpl);
                $title = "Bhukkas";
                $obj = new Widgets();
                $to =  yii::app()->functions->getOptionAdmin('alert_email_receiver');
                $from = 'info@bhukkas.com';
                $body = $obj->emailTplUserRegisterAdmin($data);
                $body1 = $obj->emailTplUserRegisterUser($data);
                // sent email to admin register detail
                if(!empty($to)){
    		 Yii::app()->functions->sendEmail($to,$from,$subject,$body,$title);  
                }
                // sent register user email
                Yii::app()->functions->sendEmail($client_email,$from,$subject,$body1,$title); 
    	}
    }
