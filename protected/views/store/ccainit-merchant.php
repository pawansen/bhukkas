...
<div class="page-right-sidebar payment-option-page">
  ...
  <?php if($status === "Aborted" || $status === "Failure"){
      
   echo "<p class='uk-alert uk-alert-danger'>Thank you for shopping with us.However,the transaction has been failed.</p>";
      
  }?>
  
  <?php $client_info = Yii::app()->functions->getClientInfo(Yii::app()->functions->getClientId());?>
  <form  name="ccaForm" id="ccaForm" method="POST" action="<?php echo Yii::app()->createUrl('/store/checkoutcca/id/'.$_GET['id']);?>" class="uk-form uk-form-horizontal" >
...
<?php echo CHtml::hiddenField('amt',$amount_to_pay)?> 
<?php echo CHtml::hiddenField('oid',$_GET['id'])?> 
